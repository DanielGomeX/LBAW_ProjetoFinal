<?php
    ...
    function getAllProductsLike($likerino) {
        ...
        $stmt = $conn->prepare("SELECT DISTINCT ON(produto.nome) produto.nome, produto.preco, produto.descricao, imagem.caminho FROM produto 
            ...
            INNER JOIN imagem ON imagem.idImagem = imagemProduto.idImagem 
            WHERE produto.nome ILIKE '%$likerino%'

            UNION

            SELECT DISTINCT ON(produto.nome) produto.nome, produto.preco, produto.descricao, imagem.caminho FROM produto 
            INNER JOIN imagemProduto ON imagemProduto.idProduto = produto.idProduto 
            INNER JOIN imagem ON imagem.idImagem = imagemProduto.idImagem 
            WHERE to_tsvector(descricao) @@ to_tsquery('$likerino');");
        $stmt->execute();
        $result = $stmt->fetchAll();
    ...
    function adicionarProduto($nomeProduto, $subcategoria, $descricaoProduto, $preco) {
        ...
        $stmt1->execute(array($subcategoria));
